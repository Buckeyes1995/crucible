...
## Benchmarking (103)
103. **Engine-vs-engine benchmark** — run the same model against the same prompt set across multiple inference engines (oMLX vs mlx_lm.server vs vLLM vs llama.cpp) and surface the results in a comparison matrix. Today the benchmark page shows tok/s / TTFT per model; this adds an "engine" dimension so users can answer "on this Mac, is oMLX or mlx_lm faster for Qwen3.6-35B at 2k context?". Handles MLX-compatible models only at first (same .safetensors can drive oMLX + mlx_lm; vLLM metal + llama.cpp need matching format conversions). Reuses the existing benchmark infra; new switch in the run config picks which engines to include.

## UX infrastructure (101–102)
...
102. **HF model detail hover card** — hovering a HuggingFace search result in `/store` pops a card with the model's description, full tags, file list, library, license, and author — pulled from `https://huggingface.co/api/models/{repo_id}` via a cached Crucible proxy (`/api/hf/model-detail/{repo_id}`, 1h TTL). Eliminates the "click out to HF in a new tab just to read the README" loop. Same component can render in `/models` for already-downloaded repos, and in Wishlist entries.
