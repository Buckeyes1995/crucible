...
type HFKind = "mlx" | "gguf" | "image" | "video";

function HFSearch({ initialQuery }: { initialQuery: string }) {
  const [query, setQuery] = useState(initialQuery);
  const [kind, setKind] = useState<HFKind>("mlx");
  ...
  const runSearch = useCallback(async (q: string, k: HFKind) => {
    if (!q.trim()) { setResults([]); return; }
    setSearching(true);
    try {
      // Text-model kinds (mlx/gguf) bias the query with a quant-keyword
      // suffix because authors tag those inconsistently. Image/video
      // kinds rely on the backend's pipeline_tag filter instead, which
      // is far more reliable for diffusion models.
      const isMedia = k === "image" || k === "video";
      const suffix = isMedia ? "" : (k === "mlx" ? " mlx" : " gguf");
      setResults(await api.hf.search(q + suffix, 30, isMedia ? k : undefined));
    } catch { setResults([]); }
    ...
    <div className="space-y-4">
      <div className="flex gap-2 items-center">
        ...
        <div className="flex rounded-lg overflow-hidden border border-white/[0.08]">
          {(["mlx", "gguf", "image", "video"] as const).map(k => (
            <button
              key={k}
              onClick={() => setKind(k)}
              className={cn(
              ...
              title={
                k === "mlx" ? "Apple-Silicon MLX models — Crucible's primary local engine"
                : k === "gguf" ? "GGUF models for llama.cpp"
                : k === "image" ? "Image-generation checkpoints (Stable Diffusion, SDXL, Flux, …) — land in your ComfyUI checkpoints dir"
                : "Video-generation checkpoints (CogVideoX, AnimateDiff, …) — land in your ComfyUI checkpoints dir"
              }
            >{k}</button>
          ))}
        </div>
